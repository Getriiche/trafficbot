...
export class PuppeteerStealthEngine implements BrowserEngine {
  ...
  async init(options: BrowserOptions): Promise<void> {
    ...
    logger.info('[DEBUG] Launching Puppeteer browser with options', { 
    ...
    logger.info('[DEBUG] Browser launched successfully');
    
    const pages = await this.browser!.pages();
    this.page = pages.length > 0 ? pages[0] : await this.browser!.newPage();
    logger.info('[DEBUG] Page created, setting up user agent');
  ...
  async close(): Promise<void> {
    if (this.browser) {
      try {
        // Set a timeout to force kill if graceful close fails
        const closeTimeout = setTimeout(async () => {
          logger.warn('Browser close timeout, forcing process termination');
          try {
            // Force kill browser process if still running
            const childProcess = (this.browser as any)?.process?.();
            if (childProcess) {
              childProcess.kill('SIGKILL');
            }
          } catch (killError) {
            logger.error('Failed to force kill browser process', { error: killError });
          }
        ...
        logger.error('Error closing browser, attempting force kill', { error });
        ...
        try {
          ...
          logger.error('Force kill also failed', { error: killError });
        }
      } finally {
        this.browser = null;
        this.page = null;
